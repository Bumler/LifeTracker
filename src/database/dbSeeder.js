...
async function seedUsers() {
  const userCollection = await getCollection(collectionNames.users);
  await seedCollection(userCollection, users);
}

async function seedTrackedFields() {
  const trackedFieldsCollection = await getCollection(collectionNames.trackedFields);
  await seedCollection(trackedFieldsCollection, trackedFields);
}

async function seedEntries() {
  const entriesCollection = await getCollection(collectionNames.entries);
